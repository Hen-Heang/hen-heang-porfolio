    ...
    return (
        <div className="px-4 md:px-8 py-10 max-w-5xl mx-auto">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-surface px-3 py-1 font-mono text-[11px] uppercase tracking-wider text-fg-secondary">
                <TestTubes size={11} aria-hidden="true" className="text-sky-600 dark:text-sky-400" />
                ~/lab/experiments
            </span>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-1.5">
                <span className="mr-1 font-mono text-[11px] uppercase tracking-wider text-fg-muted">Current focus</span>
                {currentFocus.map((f) => (
                    <Tag key={f} label={f} />
                ))}
            </div>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
                {active.map((entry) => (
                    <div key={entry.area} className="rounded-2xl border border-border bg-surface p-5">
                        ...
                        <p className="mb-3 font-mono text-xs text-brand">{entry.tech}</p>
                        <p className="text-sm leading-relaxed text-fg-muted">{entry.detail}</p>
                        {AREA_LINK[entry.area] && (
                            <Link
                                href={AREA_LINK[entry.area]}
                                className="mt-4 flex items-center gap-1 font-mono text-xs text-fg-secondary hover:text-fg transition-colors"
                            >
                                explore
