...
const METHOD_COLOR: Record<ApiEndpoint["method"], string> = {
    ...
    POST: "text-[#6366f1] border-[#6366f1]/30 bg-[#6366f1]/5",
    ...
    return (
        <div className="px-4 md:px-8 py-10 max-w-5xl mx-auto">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-[#27272a] bg-[#18181b] px-3 py-1 font-mono text-[10px] uppercase tracking-wider text-[#a1a1aa]">
                <Braces size={11} aria-hidden="true" className="text-[#6366f1]" />
                ~/lab/api
            </span>
            <h1 className="mt-4 text-3xl md:text-4xl font-bold tracking-tight text-[#fafafa]">API Design</h1>
                    ...
                    <section key={project.slug} className="rounded-2xl border border-[#27272a] bg-[#18181b] p-5 md:p-6">
                        ...
                        <div className="mt-4 space-y-2">
                            {project.apiEndpoints!.map((ep, i) => (
                                <div
                                    key={i}
                                    className="flex flex-col gap-2 rounded-xl border border-[#27272a] bg-[#0c0c0e] px-4 py-3 sm:flex-row sm:items-center"
                                >
                                    <div className="flex items-center gap-2.5 sm:w-[300px] sm:shrink-0">
                                        ...
                                        <code className="truncate font-mono text-xs text-[#fafafa]">{ep.path}</code>
                                    </div>
                                    <p className="text-xs leading-relaxed text-[#71717a]">{ep.description}</p>
                                </div>
                            ))}
                        </div>
                    </section>
