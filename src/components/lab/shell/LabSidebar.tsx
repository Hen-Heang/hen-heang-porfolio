...
const navGroups: LabNavGroup[] = [
    ...
    },
    {
        title: "Engineering",
        items: [
            { label: "System Design", icon: Network, soon: true },
            { label: "API Design", icon: Braces, soon: true },
            { label: "Database", icon: Database, soon: true },
            { label: "Performance", icon: Gauge, soon: true },
            { label: "Experiments", icon: TestTubes, soon: true },
        ],
    },
]
        ...
        <div className="flex h-full flex-col bg-[#09090b]">
            ...
            <nav className="flex-1 overflow-y-auto px-3 py-4" aria-label="Lab sections">
                {navGroups.map((group, gi) => (
                    <div key={group.title ?? gi} className={gi > 0 ? "mt-6" : ""}>
                        {group.title && (
                            <p className="mb-1.5 px-2 font-mono text-[10px] font-semibold uppercase tracking-wider text-[#52525b]">
                                {group.title}
                            </p>
                        )}
                                ...
                                if (item.soon) {
                                    return (
                                        <li key={item.label}>
                                            <span
                                                aria-disabled="true"
                                                className="flex cursor-default items-center justify-between rounded-lg px-2 py-1.5 text-[13px] text-[#3f3f46]"
                                            >
                                                <span className="flex items-center gap-2.5">
                                                    <item.icon size={15} />
                                                    {item.label}
                                                </span>
                                                <span className="rounded border border-[#27272a] px-1 py-px font-mono text-[9px] uppercase tracking-wider text-[#52525b]">
                                                    soon
                                                </span>
                                            </span>
                                        </li>
                                    ...
                                    <li key={item.label}>
                                        <Link
                                            ...
                                            className={`flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-[13px] transition-colors ${
                                                active
                                                    ? "bg-[#18181b] font-medium text-[#fafafa] shadow-[inset_2px_0_0_0_#6366f1]"
                                                    : "text-[#a1a1aa] hover:bg-[#18181b]/60 hover:text-[#fafafa]"
                                            }`}
                                        >
                                            <item.icon size={15} className={active ? "text-[#6366f1]" : "text-[#71717a]"} />
                                            {item.label}
                                        </Link>
                                    </li>
                                )
            ...
            <div className="border-t border-[#27272a] px-3 py-3 space-y-0.5">
                <a
                    ...
                    className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-[13px] text-[#a1a1aa] hover:bg-[#18181b]/60 hover:text-[#fafafa] transition-colors"
                >
