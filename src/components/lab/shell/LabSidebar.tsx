        ...
        <div className="flex h-full flex-col bg-[#09090b]">
            ...
            <div className="flex items-center gap-3 border-b border-[#27272a] px-5 py-4">
                ...
                <div className="min-w-0">
                    ...
                    <p className="truncate font-mono text-xs lg:text-[11px] text-[#52525b]">~/henheang/lab</p>
                </div>
            </div>
            ...
            <nav className="flex-1 overflow-y-auto px-3 py-4" aria-label="Lab sections">
                {navGroups.map((group, gi) => (
                    <div key={group.title ?? gi} className={gi > 0 ? "mt-6" : ""}>
                        {group.title && (
                            <p className="mb-1.5 px-2 font-mono text-[11px] lg:text-[10px] font-semibold uppercase tracking-wider text-[#52525b]">
                                {group.title}
                            </p>
                        )}
                            ...
                            {group.items.map((item) => {
                                ...
                                if (item.soon) {
                                    return (
                                        <li key={item.label}>
                                            <span
                                                aria-disabled="true"
                                                className="flex cursor-default items-center justify-between rounded-lg px-2 py-1.5 text-sm lg:text-[13px] text-[#3f3f46]"
                                            >
                                                <span className="flex items-center gap-2.5">
                                                    <item.icon size={15} aria-hidden="true" />
                                                    {item.label}
                                                </span>
                                                <span className="rounded border border-[#27272a] px-1 py-px font-mono text-[10px] lg:text-[9px] uppercase tracking-wider text-[#52525b]">
                                                    soon
                                                </span>
                                            </span>
                                        </li>
                                ...
                                return (
                                    <li key={item.label}>
                                        ...
                                        >
                                            <item.icon size={15} aria-hidden="true" className={active ? "text-[#6366f1]" : "text-[#71717a]"} />
                                            {item.label}
                                        </Link>
                                    </li>
                                )
            ...
            <div className="border-t border-[#27272a] px-3 py-3 space-y-0.5">
                <a
                    ...
                    className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-sm lg:text-[13px] text-[#a1a1aa] hover:bg-[#18181b]/60 hover:text-[#fafafa] transition-colors"
                >
