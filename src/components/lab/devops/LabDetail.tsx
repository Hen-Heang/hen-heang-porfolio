...
export function LabDetail({ lab }: { lab: Lab }) {
    return (
        <div className="text-[#d4d4d8]">
            <h2 className="mb-3 text-lg font-bold text-[#fafafa]">Prerequisites</h2>
            <ul className="mb-8 list-disc space-y-1.5 pl-5 text-[15px] leading-[1.8] text-[#a1a1aa]">
                {lab.prerequisites.map((p, i) => (
                    <li key={i}>{p}</li>
                ))}
            </ul>

            <h2 className="mb-3 text-lg font-bold text-[#fafafa]">Architecture</h2>
            <DiagramViewer diagram={lab.architecture} />

            <h2 className="mb-3 mt-8 text-lg font-bold text-[#fafafa]">Steps</h2>
            <div className="space-y-6">
                {lab.steps.map((step, i) => (
                    <div key={i}>
                        <div className="mb-2 flex items-center gap-2.5">
                            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#6366f1]/15 text-[11px] font-bold text-[#6366f1]">
                                {i + 1}
                            </span>
                            <h3 className="text-sm font-semibold text-[#fafafa]">{step.title}</h3>
                        </div>
                        {step.description && <p className="mb-2 pl-8 text-sm leading-relaxed text-[#a1a1aa]">{step.description}</p>}
                        {step.command && (
                            <div className="pl-8">
            ...
            <h2 className="mb-3 mt-8 text-lg font-bold text-[#fafafa]">Expected result</h2>
            <Callout variant="best-practice" title="What success looks like" text={lab.expectedResult} />

            <h2 className="mb-3 mt-8 text-lg font-bold text-[#fafafa]">Lessons learned</h2>
